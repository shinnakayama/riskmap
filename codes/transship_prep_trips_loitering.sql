...
---------------------------
-- modify Hanah's script "encounter_ex3_1_23_20.sql" in example queries 2020 Q1
---------------------------
...
loitering AS (
   SELECT
      ...
      avg_speed_knots
   FROM `world-fishing-827.gfw_research.loitering_events_v20200205`
   WHERE
      loitering_hours >= 8
...
merged AS (
   ...
   SELECT
      ...
      trip_end,
      gfw_trip_id
   FROM transshipment_feature) AS b
ON a.ssvid = b.carrier_ssvid
WHERE loitering_start_timestamp BETWEEN trip_start AND trip_end
   AND loitering_end_timestamp BETWEEN trip_start AND trip_end
)
